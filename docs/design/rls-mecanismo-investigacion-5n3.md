...
> **Estado: DECIDIDO — firmado en ADR-0028 (2026-07-28).** Andrea eligió la opción
> **A + B** (sin `BYPASSRLS`). Este documento pasa de "borrador pendiente" a **material
> de soporte histórico** del ADR: registra las opciones evaluadas y el test de aceptación.
> La decisión vinculante y sus consecuencias viven en
> `docs/decisions/0028-rls-mecanismo-rol-servicio.md`; la implementación en
> `db/migrations/0012_rls_mecanismo.sql` y el helper
> `packages/TripSquadExpensesPostgres/.../RolRLS.swift`.
> Referencias originales: ADR-0009 §6, ADR-0014 §3, `db/migrations/0001_expenses.sql:140-143`,
> Codex voz externa #8.
...
## Recomendación TENTATIVA → DECIDIDA (A+B, ADR-0028)
...
3. Con claims `sub=ivan` (miembro), `SELECT` ve el gasto; el ownership fino (autor, ETag) lo
   sigue haciendo la capa 1 (dominio), no la RLS.
4. Con claims `sub=ana`, la operación pasa.
5. Aserción clave: el rol `authenticated` **no** tiene `rolbypassrls`, garantizando que las
   policies se evalúan.

Implementado en `db/tests/rls_assertions.sql` (psql, gate de CI en `.github/workflows/db.yml`)
y en `packages/TripSquadExpensesPostgres/Tests/.../RLSContratoPostgresTests.swift`.

## Preguntas abiertas — resueltas por ADR-0028
- ¿A+B o barrera por-viaje (C)? → **A+B** (barrera por-usuario, capa 2 Zero-Trust).
- ¿`SET ROLE authenticated` o rol propio? → el rol de app (propio, sin bypass) es **miembro
  de `authenticated`** y asume ese rol por transacción; `private.uid()` lee el claim `sub`.
- ¿Se fija el ADR ahora? → **sí, ahora** (ADR-0028), antes de la primera escritura real,
  con el helper `enTransaccionConRol` adoptado ya en las escrituras de gastos y el resto de
  caminos como follow-up (beads).
