...
struct RoutesTests {
    ...
    func appCon(_ repo: some GastoRepositorio & Membresia & SettlementRepositorio) -> any ApplicationProtocol {
        let deps = Dependencias(
            ...
            casosReserva: CasosDeUsoReserva(repo: RepositorioEnMemoria(), itinerario: RepositorioEnMemoria(), membresia: RepositorioEnMemoria(), viajes: RepositorioEnMemoria(), estructurador: EstructuradorConfirmacionFake(datos: DatosConfirmacion(tipo: .vuelo, fechaISO: nil, numeroConfirmacion: nil, proveedor: nil))),
